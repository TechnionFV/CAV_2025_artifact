...
def read_csv_files() -> list[dataFrame]:
    ...
    for f in FILES:
        ...
        with open(path) as csvfile:
            ...
            rows = [r for r in reader]
            header: list[str] = rows[0]
            key_index = header.index(f["key column"])
            for row in rows[1:]:
                key = row[key_index]
                value = {}
                for i, h in enumerate(header):
...
def get_invariant_sizes_of_run(dfs: list[dataFrame], i: int) -> list[(str, float)]:
    f = FILES[i]
    df = dfs[i]
    t = f["invariant column"]
    depths = [(key, float(value[t])) for key, value in df.items()]
    return depths


def get_trace_size_at_each_depth_of_run(dfs: list[dataFrame], i: int) -> list[(str, float)]:
    f = FILES[i]
    df = dfs[i]
    t = f["trace column"]
    try:
        depths = [(key, value[t]) for key, value in df.items()]
        return depths
    except KeyError:
...
def get_po_size_at_each_depth_of_run(dfs: list[dataFrame], i: int) -> list[(str, float)]:
    f = FILES[i]
    df = dfs[i]
    t = f["po column"]
    try:
...
def make_invariant_size_scatter_plot(dfs: list[dataFrame], i: int, j: int):
    ...
    for (n, inv_a), (_, inv_b) in zip(a, b):
        ...
        if (math.isinf(inv_a) or math.isinf(inv_b)) and (not (math.isinf(inv_a) and math.isinf(inv_b))):
            print(x[-1], y[-1])
        # y.append(int(inv_b))
    # a = aa
    # b = bb

    # ratios = [(na, ib / max(ia, 0.00000000001)) for (na, ia), (nb, ib) in zip(a, b) if math.isfinite(ia) and math.isfinite(ib)]
    # ratios.sort(key=lambda jj: jj[1])
    # print(ratios)

    # x = [t for _, t in a]
    # y = [t for _, t in b]
    # max_length = max([g for g in x if math.isfinite(g)])
    # max_length = max(max_length, max([g for g in y if math.isfinite(g)]))
    #
    # for f in [x, y]:
    #     for k in range(len(f)):
    #         if math.isinf(f[k]):
    ...
    # plt.ylabel(title_j)
    # # plt.yscale('log')
    # plt.title(f' for {title_i} and {title_j}')
    # ax.plot(ax.get_xlim(), ax.get_ylim(), ls="--", c=".3")
    # nn = f"invariant_size_scatter_plot_{i}_and_{j}"
    # save_figure(f"{nn}.svg", close=False)
    # save_figure(f"{nn}.png")
...
def make_trace_size_scatter_plot(dfs: list[dataFrame], i: int, j: int):
    ...
    # print(f"Avg. Trace Size at last common depth ({FILES[j]["title"]}) = {sum_2 / count}")
    # print(f"Avg. Trace Size at last common depth = {sum_2 /count}")

    # ratios = [(na, ib / max(ia, 0.00000000001)) for (na, ia), (nb, ib) in zip(a, b)]
    # ratios.sort(key=lambda jj: jj[1])
    # print(ratios)
    x = [t for _, t in a]
    y = [t for _, t in b]
    ...
    # plt.ylabel(title_j)
    # # plt.yscale('log')
    # plt.title(f'Trace Size at Last Common Depth for {title_i} and {title_j}')
    # ax.plot(ax.get_xlim(), ax.get_ylim(), ls="--", c=".3")
    # nn = f"last_common_depth_trace_size_scatter_plot_{i}_and_{j}"
    # save_figure(f"{nn}.svg", close=False)
    # save_figure(f"{nn}.png")
...
def make_proof_obligation_size_scatter_plot(dfs: list[dataFrame], i: int, j: int):
    ...
    # plt.ylabel(title_j)
    # # plt.yscale('log')
    # plt.title(f'Total Proof Obligations at Last Common Depth for {title_i} and {title_j}')
    # ax.plot(ax.get_xlim(), ax.get_ylim(), ls="--", c=".3")
    # nn = f"last_common_depth_po_size_scatter_plot_{i}_and_{j}"
    # save_figure(f"{nn}.svg", close=False)
    # save_figure(f"{nn}.png")
...
def make_plots(time_limit: int, hwmcc_number: int):
    ...
    FILES = [
        {
            "path": f"./{i} HWMCC{hwmcc_number}.csv",
            "title": f"{i}",
            "key column": "model",
            "time column": "Time/Error",
            "depth column": "Depth",
